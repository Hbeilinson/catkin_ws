...
Teleop_and_Stop::Teleop_and_Stop()
{
  ROS_INFO_STREAM("got to 62");
  move_pub = n.advertise<Twist>("cmd_vel_mux/input/navi", 1, true);
  ...
  message_filters::Subscriber<sensor_msgs::LaserScan> scan_sub(n, "/scan", 1000);
  message_filters::Subscriber<geometry_msgs::TwistStamped> altered_input_sub(n, "/my_altered_input", 1000);
  TimeSynchronizer<geometry_msgs::TwistStamped, sensor_msgs::LaserScan> ats(altered_input_sub, scan_sub, 10);
...
void Teleop_and_Stop::scan_callback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
  Twist forward = Twist();
